...
async def get_buy_list(message):
    ...
    await message.answer(f'Название: {texts.text_name1} |'
                         f'Описание: {texts.text_specif1} |'
                         f'Цена: {prices.price1} руб.')

    with open('Files/1LipFactor.png', "rb") as img:  # 14
        await message.answer_photo(img)

    await message.answer(f'Название: {texts.text_name2} |'
                         f'Описание: {texts.text_specif2} |'
                         f'Цена: {prices.price2} руб.')

    with open('Files/2Calc.png', "rb") as img:  # 14
        await message.answer_photo(img)

    await message.answer(f'Название: {texts.text_name3} |'
                         f'Описание: {texts.text_specif3} |'
                         f'Цена: {prices.price3} руб.')


    with open('Files/3FitoMucil.png', "rb") as img:  # 14
        await message.answer_photo(img)

    await message.answer(f'Название: {texts.text_name4} |'
                         f'Описание: {texts.text_specif4} |'
                         f'Цена: {prices.price4} руб.')

    with open('Files/4Cabbage.png', "rb") as img:
        await message.answer_photo(img)

    await message.answer(texts.text_select_prod, reply_markup=catalog_kb)
...
async def formulas_info(call_in):
    await call_in.message.answer(texts.text_select_result)
    await call_in.answer()
